...
(define (parse-arg  a)
  (match a
    ...
    [(list x ': y '@ contract) (contract-arg (id x) (parse-type y) contract)]
...
(define (typecheck-arg a envT funs)
  (match a
    [(arg id type) type]
    [(contract-arg id type contract) type]
    ))
...
(define (generate-new-env env the-args args-expr funs) 
  (foldl (λ (args val e) (match args
                           [(arg i type)
                            (def (id x) i)
                            (extend-env x (interp val env funs) e)]
                           [(contract-arg i type contract)
                            (def (id x) i)
